...
1. **实现游戏引擎集成** (`GameEngine.cpp`)

   - 整合各个子系统（FruitGenerator, MatchDetector, FallProcessor, ScoreCalculator）
   - 实现游戏主循环
   - 实现交换水果逻辑
   - 实现消除-下落-再消除的循环

2. **实现地图管理器** (`MapManager.cpp`)
...
3. **编写游戏引擎单元测试** (`tests/test_game_engine.cpp`)
   - 测试游戏初始化
   - 测试水果交换
   - 测试消除计分
   - 测试连锁消除
